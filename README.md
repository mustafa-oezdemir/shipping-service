...
An independent Go/Gin logistics **microservice** for NordShop that works together with the main `ecommerce-gin` application through authenticated REST APIs. It owns shipment tracking, delivery events, labels, QR codes, and operational logistics data while keeping its own database; it does **not** read from or write to the e-commerce database directly.
